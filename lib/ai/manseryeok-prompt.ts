...
export async function generateManseryeok(
  ...
  const monthlyPillarsM = Array.from({ length: 12 }, (_, i) => {
    ...
    if (gongmangJiListM.includes(mJi)) {
      ...
      if (resolvesM.length > 0) adjNotes.push('공망해소(+3점)')
        ...
        const hapResolvesM = gongmangJiListM.filter((gji: string) =>
          YUKHAP_M.some(([a, b]) => (mJi === a && gji === b) || (mJi === b && gji === a))
        )
        if (hapResolvesM.length > 0) adjNotes.push('공망합해소(+2점)')
      }
    }
  ...
  // 세운 지지 ↔ 사주 지지(일지/년지/월지) 충합 사전 계산 (AI 판단 → 코드 계산값)
  const seunJiM = seunPillarM[1]
  const seunJiChungHapPartsM: string[] = []
  // 일지(日支)
  if (CHUNG_M.some(([a, b]) => (seunJiM === a && saju.dayPillar[1] === b) || (seunJiM === b && saju.dayPillar[1] === a))) {
    seunJiChungHapPartsM.push(`일지충(日支冲): 세운 지지 ${seunJiM} ↔ 일지 ${saju.dayPillar[1]} 충 — 개인 운명 직격, currentSeun score -8점 기준`)
  } else if (YUKHAP_M.some(([a, b]) => (seunJiM === a && saju.dayPillar[1] === b) || (seunJiM === b && saju.dayPillar[1] === a))) {
    seunJiChungHapPartsM.push(`일지합(日支合): 세운 지지 ${seunJiM} ↔ 일지 ${saju.dayPillar[1]} 합 — 개인 운명 길조, currentSeun score +6점 기준`)
  } else {
    seunJiChungHapPartsM.push(`일지 충합 없음 (세운 지지 ${seunJiM} ↔ 일지 ${saju.dayPillar[1]})`)
  }
  // 년지(年支)
  if (CHUNG_M.some(([a, b]) => (seunJiM === a && saju.yearPillar[1] === b) || (seunJiM === b && saju.yearPillar[1] === a))) {
    seunJiChungHapPartsM.push(`년지충(年支冲): 세운 지지 ${seunJiM} ↔ 년지 ${saju.yearPillar[1]} 충 — 근본 기운 흔들림, score -4점 기준`)
  } else if (YUKHAP_M.some(([a, b]) => (seunJiM === a && saju.yearPillar[1] === b) || (seunJiM === b && saju.yearPillar[1] === a))) {
    seunJiChungHapPartsM.push(`년지합(年支合): 세운 지지 ${seunJiM} ↔ 년지 ${saju.yearPillar[1]} 합 — 근본 기운 안정, score +3점 기준`)
  }
  // 월지(月支)
  if (CHUNG_M.some(([a, b]) => (seunJiM === a && saju.monthPillar[1] === b) || (seunJiM === b && saju.monthPillar[1] === a))) {
    seunJiChungHapPartsM.push(`월지충(月支冲): 세운 지지 ${seunJiM} ↔ 월지 ${saju.monthPillar[1]} 충 — 사회·직업 기운 방해, score -5점 기준`)
  } else if (YUKHAP_M.some(([a, b]) => (seunJiM === a && saju.monthPillar[1] === b) || (seunJiM === b && saju.monthPillar[1] === a))) {
    seunJiChungHapPartsM.push(`월지합(月支合): 세운 지지 ${seunJiM} ↔ 월지 ${saju.monthPillar[1]} 합 — 사회·직업 기운 협력, score +4점 기준`)
  }
  const seunJiChungHapNoteM = seunJiChungHapPartsM.join('\n   ')
...
2. ${targetYear}년 세운 (年運): ${seunPillarM}(${seunPillarHanjaM}) — currentSeun.ganji는 반드시 "${seunPillarM}"으로 설정하세요.
   ...
   [세운 지지 충합 사전 계산값 — 반드시 이 값을 기준으로 score 산정]:
   ${seunJiChungHapNoteM}
   [세운 천간 × 대운 천간 교차분석]:
   ${(() => {
