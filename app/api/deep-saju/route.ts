...
import { generateDeepSaju } from '@/lib/ai/deep-saju-prompt'
...
export async function POST(req: NextRequest) {
  try {
    ...
    const analysis = await generateDeepSaju(birthDate, saju, detail, gender)

    // Only save to DB if logged in
    if (user) {
      const { error: saveError } = await supabase.from('deep_saju_readings').insert({
        user_id: user.id,
      ...
      if (saveError) {
        console.error('DeepSaju DB error:', saveError)
      }
    }

    return NextResponse.json({ analysis, cached: false })
  } catch (error) {
    console.error('DeepSaju API error:', error)
    return NextResponse.json(
      { error: '심층사주 분석 중 오류가 발생했습니다' },
      { status: 500 }
    )
  }
}
