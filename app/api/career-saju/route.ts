...
import { generateCareerSaju } from '@/lib/ai/career-saju-prompt'
...
export async function POST(req: NextRequest) {
  try {
    ...
    const analysis = await generateCareerSaju(birthDate, saju, gender)

    // Only save to DB if logged in
    if (user) {
      const { error: saveError } = await supabase.from('career_readings').insert({
        user_id: user.id,
      ...
      if (saveError) {
        console.error('CareerSaju DB error:', saveError)
      }
    }

    return NextResponse.json({ analysis, cached: false })
  } catch (error) {
    console.error('CareerSaju API error:', error)
    return NextResponse.json(
      { error: '직업 적성 분석 중 오류가 발생했습니다' },
      { status: 500 }
    )
  }
}
