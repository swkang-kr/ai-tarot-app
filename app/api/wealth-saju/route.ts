...
import { generateWealthSaju } from '@/lib/ai/wealth-saju-prompt'
...
export async function POST(req: NextRequest) {
  try {
    ...
    const analysis = await generateWealthSaju(birthDate, saju, gender)

    // Only save to DB if logged in
    if (user) {
      const { error: saveError } = await supabase.from('wealth_readings').insert({
        user_id: user.id,
      ...
      if (saveError) {
        console.error('WealthSaju DB error:', saveError)
      }
    }

    return NextResponse.json({ analysis, cached: false })
  } catch (error) {
    console.error('WealthSaju API error:', error)
    return NextResponse.json(
      { error: '재물운 분석 중 오류가 발생했습니다' },
      { status: 500 }
    )
  }
}
