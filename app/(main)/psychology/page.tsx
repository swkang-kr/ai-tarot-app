...
export default function PsychologyPage() {
  ...
  const [result, setResult] = useState<PsychologyResponse | null>(null)
  const [error, setError] = useState<string | null>(null)

  useEffect(() => {
    // 이전에 입력한 생년월일 복원
    const saved = sessionStorage.getItem('psych_birth_date')
    if (saved) setBirthDate(new Date(saved))
  }, [])

  const handleBirthDateChange = (date: Date | null) => {
    setBirthDate(date)
    if (date) sessionStorage.setItem('psych_birth_date', date.toISOString())
    else sessionStorage.removeItem('psych_birth_date')
  }
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(d.error || '사주 심리 분석에 실패했습니다')
      }
      ...
      <div className="max-w-md mx-auto space-y-6">
        <AnimatePresence mode="wait">
            ...
            >
              ...
              <div className="bg-white/10 backdrop-blur-lg rounded-3xl p-8 shadow-2xl">
                <div className="text-center mb-7">
                  ...
                  <h1 className="text-2xl font-bold text-white mb-1">사주 심리 분석</h1>
                  <p className="text-violet-300 text-sm">사주로 알아보는 나의 심리 유형</p>
                </div>
                ...
                <div className="mb-6">
                  ...
                  <select
                    ...
                    className="w-full bg-white/10 border border-white/20 text-white rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-violet-400 appearance-none"
                  >
                ...
                <motion.button
                  ...
                  disabled={!birthDate}
                  className="w-full bg-gradient-to-r from-violet-500 to-fuchsia-500 text-white py-4 rounded-xl font-semibold text-lg shadow-lg disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  🔮 사주 심리 분석하기
                </motion.button>
              </div>
          ...
          {step === 'result' && result && (
            ...
            >
              <PsychologyCard analysis={result} birthDate={birthDateStr} />
              <motion.div
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
