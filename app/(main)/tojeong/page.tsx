...
import TojeongCard from '@/components/TojeongCard'
...
export default function TojeongPage() {
  ...
  const [result, setResult] = useState<TojeongResponse | null>(null)
  ...
  const doSubmit = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(d.error || '토정비결 생성에 실패했습니다')
      }
      const d = await res.json()
        ...
        <AnimatePresence mode="wait">
            ...
            >
              <div className="bg-white/10 backdrop-blur-lg rounded-3xl p-8 shadow-2xl">
                <div className="text-center mb-7">
                  ...
                  <h1 className="text-2xl font-bold text-white mb-1">토정비결</h1>
                  <p className="text-orange-300 text-sm">144괘로 풀어보는 올 한 해 운세</p>
                </div>
                ...
                <div className="mb-6">
                  ...
                  <select
                    ...
                    className="w-full bg-white/10 border border-white/20 text-white rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-orange-400 appearance-none"
                  >
                ...
                <motion.button
                  ...
                  disabled={!birthDate}
                  className="w-full bg-gradient-to-r from-orange-500 to-amber-500 text-white py-4 rounded-xl font-semibold text-lg shadow-lg disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  ☯️ 토정비결 풀이하기
                </motion.button>
              </div>
          ...
          {step === 'result' && result && (
            ...
            >
              ...
              <TojeongCard analysis={result} targetYear={targetYear} />
              <motion.div
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
