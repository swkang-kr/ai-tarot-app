...
import WealthSajuCard from '@/components/WealthSajuCard'
...
export default function WealthSajuPage() {
  ...
  const [result, setResult] = useState<WealthSajuResponse | null>(null)
  ...
  const doSubmit = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch('/api/wealth-saju', {
        method: 'POST',
        ...
        <AnimatePresence mode="wait">
            ...
            >
              <div className="bg-white/10 backdrop-blur-lg rounded-3xl p-8 shadow-2xl">
                <div className="text-center mb-7">
                  ...
                  <h1 className="text-2xl font-bold text-white mb-1">재물운 심층 분석</h1>
                  <p className="text-yellow-300 text-sm">사주 재성·식상으로 보는 나의 재물 패턴</p>
                </div>
                ...
                <div className="mb-6">
                  ...
                  <select
                    ...
                    className="w-full bg-white/10 border border-white/20 text-white rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-yellow-400 appearance-none"
                  >
                ...
                <motion.button
                  ...
                  className="w-full bg-gradient-to-r from-yellow-500 to-amber-500 text-white py-4 rounded-xl font-semibold text-lg shadow-lg disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  💰 재물운 분석하기
                </motion.button>
              </div>
          ...
          {step === 'result' && result && (
            <motion.div key="result" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}>
              ...
              <WealthSajuCard analysis={result} birthDate={birthDate?.toISOString().split('T')[0] ?? ''} />
