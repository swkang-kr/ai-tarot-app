...
import CareerSajuCard from '@/components/CareerSajuCard'
...
export default function CareerSajuPage() {
  const [birthDate, setBirthDate] = useState<Date | null>(null)
  const [birthHour, setBirthHour] = useState<number | null>(null)
  const [step, setStep] = useState<'input' | 'loading' | 'result'>('input')
  const [result, setResult] = useState<CareerSajuResponse | null>(null)
  ...
  const doSubmit = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch('/api/career-saju', {
        method: 'POST',
        ...
        <AnimatePresence mode="wait">
            ...
            >
              <div className="bg-white/10 backdrop-blur-lg rounded-3xl p-8 shadow-2xl">
                <div className="text-center mb-7">
                  ...
                  <h1 className="text-2xl font-bold text-white mb-1">직업·적성 분석</h1>
                  <p className="text-blue-300 text-sm">사주로 알아보는 나의 최적 커리어 패스</p>
                </div>
                ...
                <div className="mb-6">
                  ...
                  <select
                    ...
                    className="w-full bg-white/10 border border-white/20 text-white rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-blue-400 appearance-none"
                  >
                ...
                <motion.button
                  ...
                  onClick={handleSubmit} disabled={!birthDate}
                  className="w-full bg-gradient-to-r from-blue-500 to-sky-500 text-white py-4 rounded-xl font-semibold text-lg shadow-lg disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  🚀 직업 적성 분석하기
                </motion.button>
              </div>
          ...
          {step === 'result' && result && (
            <motion.div key="result" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}>
              {sajuInfo && <div className="mb-6"><SajuCard saju={sajuInfo} /></div>}
              <CareerSajuCard analysis={result} birthDate={birthDate?.toISOString().split('T')[0] ?? ''} />
